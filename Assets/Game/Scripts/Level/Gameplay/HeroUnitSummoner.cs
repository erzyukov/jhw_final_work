namespace Game.Level
{
	...
	public class HeroUnitSummoner : ControllerBase, IHeroUnitSummoner, IInitializable
	{
		...
		Dictionary<IUnitFacade, IDisposable>	_unitDiedSubscribes		= new();
		Dictionary<IUnitFacade, IDisposable>	_vanishSubscribes		= new();
		Dictionary<Species, int>				_speciesCounter			= new();
		...
		public ReactiveCommand					SummoningPaidUnit	{ get; } = new();
		public ReactiveCommand<IUnitFacade>		SummonedPaidUnit	{ get; } = new();
		...
		private Species GetRandomSpecies()
		{
			...
			List<Species> species	= _gameProfile.Squad.ToList();

			_gameProfile.Squad.ForEach( s =>
			{
		...
		private void InitCounter() =>
			_gameProfile.Squad.ForEach( species => _speciesCounter.Add( species, 0 ) );

		private void UpdateCounter()
		{
			_gameProfile.Squad.ForEach( species => _speciesCounter[species] = 0 );
